...
namespace WebAppServer.Repositories
{
    public class GrupoRepository
    {
        ...
        public async Task<string> GravarDados(string operacao, string dados)
        {
            ...
            List<Grupo> listDados = new List<Grupo>();
            string str_param = "";
            ...
            try
            {
                listDados = JsonConvert.DeserializeObject<List<Grupo>>(str_param.IndexOf("[") == -1 ? "[" + str_param + "]" : str_param);
                ...
                if (listDados != null)
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(str_conn))
                    {
                        ...
                        using (SqlTransaction tran = conn.BeginTransaction())
                        {
                            try
                            {
                                id_ret = Convert.ToInt64(repData.ManutencaoTabela<Grupo>(operacao, listDados, "ntv_tbl_grupo", conn, tran).Split(";")[0]);
                                grupo = listDados[0];
                                grupo.id = id_ret;
                                str_ret = JsonConvert.SerializeObject(grupo);
                            }
                            catch (Exception ex)
                            {
                                tran.Rollback();
